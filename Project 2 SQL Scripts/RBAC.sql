...
SET @name='suyash';
SET @pass='suyash';
CALL `student_clinic_db`.`user_auth`(@name, @pass, @role);
SELECT @role;
...
SET @med_name = 'Calpol';
SET @med_quantity = 2;
CALL `student_clinic_db`.`create_prescription`(@username, @pass, @presc_id, @med_name, @med_quantity);
...
SET @pass = 'abhishek';
SET @presc_id = 95;
CALL `student_clinic_db`.`delete_prescription`(@username, @pass, @presc_id);
